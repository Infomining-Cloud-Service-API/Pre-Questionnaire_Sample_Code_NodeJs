...
var step2HistoryResponseModel = require('../model/step2/step2_history_response_model')
...
function main() {
    ...
    //     // question_id = constant.step2_question_id,
    //     // selection_id = constant.step2_selection_id,
    //     // input_txt = 'test',
    // )
    // getHistory(
    //     accessToken = constant.accessToken,
...
/**
 * @apiNote
 *   Step2 Question (Save your answers and view the following questions)
 *
 ...
 * @param questionId <- Step2 identifier of the question (optional)
...
function getQuestion(accessToken, report_id, language_type, question_id, selection_id, input_txt) {
    ...
    const options = {
        uri: constant.step1ScenarioUrl + '/v1/step2/question',
        headers: {
...
/**
 * @apiNote
 *   Step2 History (View the list of values I have selected)
 *
 * @param accessToken <- Your_Access_Token (required)
...
function getHistory(accessToken, report_id) {
    ...
    const options = {
        uri: constant.step1ScenarioUrl + '/v1/step2/history',
        headers: {
            'Authorization': 'Bearer ' + accessToken,
            'Content-Type': constant.contentType,
        },
    ...
    request.post(options, function (e, response, body) {
        const base = new baseResponseModel.BaseResponseModel(JSON.parse(response.body), step2HistoryResponseModel.Step2HistoryResponseModel);
        console.log(JSON.stringify(base));
    });
}